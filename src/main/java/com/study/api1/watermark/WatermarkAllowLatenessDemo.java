...
public class WatermarkAllowLatenessDemo {
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<String> process = singleOutputStreamOperator
                .keyBy(WaterSensor::getId)
                .window(TumblingEventTimeWindows.of(Time.seconds(10)))
                .allowedLateness(Time.seconds(2)) // 允许迟到数据，2秒 当等待时间结束之后，再下一条事件时间<=窗口时间+等待时间+允许迟到时间-1ms时的数据都能进入窗口
                .sideOutputLateData(lateData)// 迟到数据输出到侧输出流
        ...
        process.getSideOutput(lateData).printToErr("迟到数据");
        process.print();
        env.execute();
    }
}
